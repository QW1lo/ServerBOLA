...
int main() {
    ...
    while (1)
	{
		...
		if (table[pack1.bits.adress].name == "слово БИНС")
		{
			...
			fprintf_s(ofile, "\n%-35s", "  ");
			fprintf_s(ofile, "%-35s", "Испавность ИНС");
		...
		if (table[pack1.bits.adress].name != "слово СНС" && table[pack1.bits.adress].name != "слово БИНС" && table[pack1.bits.adress].name != "дата")
		{
			...
			fprintf_s(ofile, "\n%-35s", "  ");
			fprintf_s(ofile, "%-35s", "P");
			fprintf_s(ofile, "%-35d", pack1.bits.P);
			*/
	...
	/*ARINC_BNR pack;               // Формирование 6-го слова
	pack.bits.adress = 110;       // 120 - текущая широта(точно)
	pack.bits.value = 140;		  // Тут должн астоять твоя фунция из бинарного массива в int 
	pack.bits.sign = 1;
	pack.bits.SM = 2;
	pack.bits.P = 1;
